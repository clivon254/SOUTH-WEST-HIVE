...
export default function UpdateReels() {
    ...
    const fetchReel = async () => {
        ...
        try
        {
            ...
            const res = await axios.get(url + `/api/reel/get-reel/${reelId}`)

            if(res.data.success)
            {
                setFetching(false)

                setFormData(res.data.reel)
            }
        }
        catch(error)
        {
  ...
  return (
    ...
    <section className="section space-y-10 max-w-xl mx-auto">
        ...
        <h1 className="title text-center">Update Reel</h1>
